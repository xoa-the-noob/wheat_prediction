...
def setup_logging(base_dir="logs2"):
...
def load_client_csv(csv_path: Path, harmonize_yield_units=True):
    ...
    if len(feature_cols) == 0:
        logging.warning(
            f"{csv_path.name}: no valid numeric features; using dummy feature"
        )
        df["__dummy__"] = 0.0
...
def train_local_proximal(
    ...
    *,
    ...
    global_tensors = [torch.tensor(w, device=device) for w in global_weights]
...
class WheatClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_loss = train_local_proximal(
            self.model,
            parameters,
            self.cdata.X_train,
            self.cdata.y_train,
        ...
        yhat_train = predict(self.model, self.cdata.X_train, device=self.device)
        train_mae = float(mean_absolute_error(self.cdata.y_train, yhat_train))

        num_examples = self.cdata.X_train.shape[0]
        metrics = {
            "train_loss": train_loss,
            "train_mae": train_mae,
            "client_name": self.cdata.name,
        }
...
def run_local_baselines(
    clients: List[ClientData], device: str = "cpu"
...
def run_centralized_baseline(
    ...
    train_local(
        model, X_train, y_train, epochs=50, lr=1e-3, batch_size=64, device=device
    )
...
class FedProxWithParams(fl.server.strategy.FedProx):
    def __init__(self, clients_data: List[ClientData], device: str, *args, **kwargs):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        # Per-client train MAE from fit metrics
        client_train_mae = {}
        for _client_proxy, fit_res in results:
            name = str(fit_res.metrics.get("client_name", "unknown"))
            client_train_mae[name] = float(
                fit_res.metrics.get("train_mae", float("nan"))
            )

        # Evaluate global model on val sets
        ndarrays = fl.common.parameters_to_ndarrays(params)
        global_model = MLPRegressor(in_dim=self.clients_data[0].X_train.shape[1]).to(
            self.device
        )
        set_model_weights(global_model, ndarrays)

        # Per-client val MAE with global model
        client_val_mae = {}
        for c in self.clients_data:
            yhat_c = predict(global_model, c.X_val, device=self.device)
            client_val_mae[c.name] = float(mean_absolute_error(c.y_val, yhat_c))

        # Pooled global val MAE
        X_val_all = np.concatenate([c.X_val for c in self.clients_data], axis=0)
        y_val_all = np.concatenate([c.y_val for c in self.clients_data], axis=0)
        yhat_val_all = predict(global_model, X_val_all, device=self.device)
        global_val_mae = float(mean_absolute_error(y_val_all, yhat_val_all))

        # Pooled global train MAE
        X_train_all = np.concatenate([c.X_train for c in self.clients_data], axis=0)
        y_train_all = np.concatenate([c.y_train for c in self.clients_data], axis=0)
        yhat_train_all = predict(global_model, X_train_all, device=self.device)
        global_train_mae = float(mean_absolute_error(y_train_all, yhat_train_all))
        ...
        self.round_history.append(
            {
                ...
                "client_train_mae": client_train_mae,
                "client_val_mae": client_val_mae,
            }
        )

        train_str = "  ".join(
            f"{k}={v:.2f}" for k, v in sorted(client_train_mae.items())
        )
        val_str = "  ".join(f"{k}={v:.2f}" for k, v in sorted(client_val_mae.items()))
        logging.info(
            f"[Round {server_round:3d}] Global Train MAE={global_train_mae:.2f}  Global Val MAE={global_val_mae:.2f} | "
            f"Train MAE: {train_str} | Val MAE: {val_str}"
        )
...
def run_federated_with_params(
    ...
    *,
    ...
    proximal_mu: float = 1.0,
    device: str = "cpu",
) -> Tuple[Dict[str, float], Dict[str, Dict[str, float]], List[Dict], List[np.ndarray]]:
    logging.info(
        f"FL config: rounds={rounds}  local_epochs={local_epochs}  "
        f"proximal_mu={proximal_mu}  lr=0.001  batch_size=32  clients={len(clients)}"
    )
    ...
    # Evaluate pooled + per-client on test set
    ...
    return pooled, per_client, strategy.round_history, ndarrays
...
def run_personalized(
    ...
    *,
    ...
    """Fine-tune the global model on each client's local train set, evaluate on test set."""
    in_dim = clients[0].X_train.shape[1]
    per_client = {}
    for c in clients:
        model = MLPRegressor(in_dim=in_dim).to(device)
        set_model_weights(model, global_weights)
        train_local(model, c.X_train, c.y_train, epochs=finetune_epochs, lr=1e-3, batch_size=32, device=device)
        yhat = predict(model, c.X_test, device=device)
        per_client[c.name] = regression_metrics(c.y_test, yhat)
    return per_client
...
def print_table(title: str, rows: List[Tuple[str, Dict[str, float]]]) -> None:
    ...
    for name, mets in rows:
        print(f"{name:<20} " + " ".join([f"{mets[c]:12.4f}" for c in cols]))
...
def main():
    ...
    logging.info(
        f"Config: dataset_dir={args.dataset_dir}  rounds={args.rounds}  "
        f"local_epochs={args.local_epochs}  mu={args.mu}  device={args.device}  "
        f"harmonize_yield={harmonize}"
    )
    ...
    for c in clients:
        logging.info(
            f"  {c.name}: train={c.X_train.shape[0]}  val={c.X_val.shape[0]}  "
            f"test={c.X_test.shape[0]}  features={c.X_train.shape[1]}"
        )
    ...
    pooled_fl, per_client_fl, round_history, global_weights = run_federated_with_params(
        clients,
        rounds=args.rounds,
    ...
    log_metrics_table("FEDERATED POOLED", {"fedprox": pooled_fl})
    ...
    save_json(round_history, run_dir / "metrics_federated_rounds.json")

    # ---- PERSONALIZED (fine-tune global model per client) ----
    logging.info(f"Running PERSONALIZED fine-tuning (finetune_epochs={args.finetune_epochs})...")
    personalized = run_personalized(
        clients,
        global_weights,
    ...
    log_metrics_table("PERSONALIZED (FedProx + finetune)", personalized)
    save_json(personalized, run_dir / "metrics_personalized.json")
