...
class WheatClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        metrics = {"train_loss": train_loss, "train_mae": train_mae, "client_name": self.cdata.name}
...
class FedAvgWithParams(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        for _client_proxy, fit_res in results:
            ...
            client_train_mae[name] = float(fit_res.metrics.get("train_mae", float("nan")))

        # Evaluate global model on val sets
        ndarrays = fl.common.parameters_to_ndarrays(params)
        global_model = MLPRegressor(in_dim=self.clients_data[0].X_train.shape[1]).to(self.device)
        ...
        train_str = "  ".join(f"{k}={v:.2f}" for k, v in sorted(client_train_mae.items()))
...
def run_federated_with_params(
    ...
    *,
    ...
    logging.info(
        ...
        f"lr=0.001  batch_size=32  clients={len(clients)}"
    )
    ...
    return pooled, per_client, strategy.round_history
...
def main():
    ...
    logging.info(
        ...
        f"local_epochs={args.local_epochs}  device={args.device}  "
    ...
    pooled_fl, per_client_fl, round_history = run_federated_with_params(
        clients,
        rounds=args.rounds,
